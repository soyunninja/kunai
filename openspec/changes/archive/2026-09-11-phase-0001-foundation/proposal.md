...
4. **Server-side PocketBase configuration validation.** Prepare one small, consistent server-side endpoint configuration and validation boundary. Document the local PocketBase endpoint and how to run or supply the local service. This foundation work does not choose the future PocketBase client/server access strategy, including authentication and ordinary user-data access; Phase 0002 owns that decision. Do not introduce the PocketBase SDK unless a demonstrated Foundation requirement makes it necessary. Secret-requiring operations remain server-side in all later strategies.
5. **Environment and secret discipline.** Define the environment-variable strategy and add `.env.example` when code is introduced. Expose the PocketBase endpoint publicly only if an approved later access strategy requires browser access; Phase 0001 should keep configuration private unless implementation proves otherwise and documents why. Keep all secret values out of public runtime configuration, browser bundles, logs, examples, and source control. Do not add unused provider credential infrastructure.
...
| PocketBase/runtime configuration | Server-side endpoint configuration validation and explicit public/private classification; Phase 0002 retains the client/server access decision. |
...
- **Premature architecture:** keep the server-side configuration boundary narrow. Do not infer that all future PocketBase access is server-side; Phase 0002 selects the auth and ordinary user-data access strategy, while secret-requiring operations remain server-side.
...
- **Review size:** the generated `pnpm-lock.yaml` is excluded from the manual 400-line review limit. The parent must assess the remaining implementation by conceptual cohesion and risk during task planning and honor `ask-on-risk`; lockfile size alone does not justify chaining or a size exception.
...
7. Implementation and verification evidence cover only Phase 0001 behavior; deferred scope is controlled by this proposal, the specifications, design, and tasks rather than absence-only tests.
